...
func _ready() -> void:
	var err = 0
	err += $FirstCards/Button.connect("mouse_entered", self, "highlight")
	err += $FirstCards/Button.connect("mouse_exited", self, "unhighlight")
	err += $FirstCards/Button.connect("pressed", self, "clicked")
	if err:
...
func set_card(c: String):
	...
	if c == "blank":
		...
		$FirstCards.frame = 54 if sprite_frame < 0 else sprite_frame
	card = c


func set_sprite_rotation(degrees: int) -> void:
	$FirstCards.rotation_degrees = degrees
